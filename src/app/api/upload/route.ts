...
// Uploads are filed under the app that's currently open, so two apps can hold
// screenshots with identical content without sharing a folder.
function uploadDirRel(appId: string) {
  return path.join("public", "screenshots", appId, "uploaded");
}
...
export async function POST(req: Request) {
  ...
  const absDir = path.join(process.cwd(), uploadDirRel(appId));
  ...
  try {
    ...
    return NextResponse.json({ ok: true, path: `${appUploadPrefix(appId)}/${filename}` });
  } catch (e) {
    return NextResponse.json(
