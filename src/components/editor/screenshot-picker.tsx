...
type Props = {
  label: string;
  // Which app's uploads folder receives the file.
  appId: string;
  value: string;
  locale?: string;
...
async function uploadDataUrl(dataUrl: string, appId: string): Promise<string | null> {
  try {
    const resp = await fetch("/api/upload", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ dataUrl, app: appId }),
    });
...
export function ScreenshotPicker({ label, appId, value, locale, onChange }: Props) {
